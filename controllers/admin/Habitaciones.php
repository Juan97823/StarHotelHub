...
class Habitaciones extends Controller
{
    ...
    public function guardar()
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $estilo = $_POST['estilo'] ?? '';
            $numero = $_POST['numero'] ?? 0; // Este campo no se usa en el form, pero lo mantenemos por si acaso
            ...
            if ($foto && $foto['name'] != '') {
                ...
                if (move_uploaded_file($foto['tmp_name'], $destino . $nombre_foto)) {
                    ...
                    $nombre_foto = $foto_actual; // Si falla la subida, mantenemos la foto anterior
                }
            }
    ...
    public function eliminar($id)
    {
        ...
        if ($data >= 1) {
            ...
            $res = ['msg' => 'Error al dar de baja', 'icono' => 'error'];
        }
    ...
    public function reingresar($id)
    {
        ...
        if ($data >= 1) {
            $res = ['msg' => 'Habitación reingresada', 'icono' => 'success'];
        } else {
            $res = ['msg' => 'Error al reingresar', 'icono' => 'error'];
        }
        echo json_encode($res, JSON_UNESCAPED_UNICODE);
        die();
    }
